...
const loadMore = async () => {
  try {
    ...
    const{data} = await axios.get(`http://localhost:5000/api/product/product-list/${page}`)
    setLoading(false)
      // جبلي البرودكتس يلي قبل الضغط على اللود مور وبالاضافة للبرودكتس بعد الضغط على اللود مور 
...
const filteredProducts = async () => {
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.header}> MENUE</h1>
      <div className={styles.filters}>
        <button onClick={ () => window.location.reload() }>Reset</button>
    
        <div className={styles.searchProduct} >
          <input value={search.toLowerCase()} onChange={e => setSearch(e.target.value)} placeholder='Search name ..'/>   
        </div>
